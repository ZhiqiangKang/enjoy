...
public class InsuranceContract {
    ...
    public static class ConcreteBuilder{
        ...
        public InsuranceContract build(){
            ...
            boolean personSign = (personName != null && personName.trim().length() > 0);
            boolean companySign = (companyName != null && companyName.trim().length() > 0);
            if(personSign && companySign){
                throw new IllegalArgumentException("一份保险合同不能同时与个人和公司签订");
            }
            if(personSign == false && companySign == false){
